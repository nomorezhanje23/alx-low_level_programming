...
/**
 * print_magic -this prints the magic numz of an ELF header
 * @e_ident: a pointer to an array containing the ELF magic numz
 * Description: the magic numz being separated by spaces
 */
...
/**
 * print_class - this prints the class ELF header
...
/**
 * print_data - this the ELF header
 ...
 * @e_ident: pointer array with ELF version
 */
void print_version(unsigned char *e_ident)
{
printf(" Version: %d",
 ...
 * @e_type: ELF type
 * @e_ident: pointer with ELF class
 */
void print_type(unsigned int e_type, unsigned char *e_ident)
{
...
/**
 * close_elf - this closes ELF file
 * @elf: file descriptor of  ELF file
 * Description: when the file cannot be closed then exit code 98
 */
void close_elf(int elf)
{
if (close(elf) == -1)
...
/**
 * main - displays the ELF header info at the start of an ELF file
 * @argc: the numb of arguments supplied to the C prog
